...
export function GoogleSignInButton({ className, mode = 'login' }: GoogleSignInButtonProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/${locale}/auth/callback-client`,
          queryParams: {
            access_type: 'offline',
            prompt: 'consent'
          }
      ...
      // The redirect will happen automatically, errors are rare here
    } finally {
      setIsLoading(false);
    }
  };
